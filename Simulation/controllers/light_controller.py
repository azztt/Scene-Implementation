...
class LightController(Controller):
    def __init__(self, name: str, id: str) -> None:
        super().__init__(name, id, DeviceType.LIGHT)
        self.__running = False
        self.__client: MQTTConnection = None
        self.__status_thread: StatusThread = None
    
    def error(self, errmsg: str, light: Light = None) -> None:
        """
        Prints/logs error message
        """
        prefix = ""
        if light:
            prefix = "LightController for light {}->".format(
                light.get_name()
            )
        else:
            prefix = "LightController->"
    ...
    def set_brightness(self, light_id: str, level: int) -> Literal:
        ...
        """
        try:
            light: Light = self.get_device_by_id(light_id)
            if light:
    ...
    def __on_message(self,
        ...
        if device_id in self.get_all_device_ids().split(","):
            ...
            elif command == LightOp.SET_BRIGHT.value:
                level = int(message[2])
